...
jumbotron:
  ...
  alt: computing
publishedDate: 2023-05-04T21:00:00+10:00
---
...
It was easy enough for me to create a cluster of computers using EC2 batch, but
the processes are fairly involved if you're trying to run something fast and
cheap. I found a better way of solving the problem by using AWS Step Functions
...
results together. The architecture diagram below shows how this project will
compute the result.
...
This is a basic CDK stack to create and deploy lambda functions. The
...
download the file and then return the sum the results.
...
which is a high-concurrency mode designed to process a larger number of
functions. To use this in the stack we need to create an S3 bucket to store the
results of the map.
