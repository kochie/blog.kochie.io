...
with their morals and values more accuratly[^3]. This means that companies can
...
In Austrlia the "big 4" major banks have roughly 73% of the Australian banking
...
updates, you'll find it's handle on twitter
...
a lot of banking executuves doing. Exposing yourself on Twitter is a pretty
...
had a simple announcement stating there was a large scale event, Up sent hourly
updates to its customers keeping them informed - some banks didn't event notify
cutomers of the outage!

This is done via their in-app communication panel which allows customer support
staff to talk to customers live without the neext for a branch. Where
traditional banks require a customer to travel to a branch or pick up the phone
to get support all Up custoemrs do is start a DM. Additionaly the process is
asynchronous - so you can do other things while you're waiting for a response.
This is how disruption benifits consumers, while Up wasn't the first company to
...
have to cut the price of your product which is exatly what Athena does.
...
>
  <div>
    [Hmm,] I was pretty amped for a bank building a new take on banking, and
    delivering quickly at the same time.
  </div>
  <div>
...
innovating it will enventually be disrupted. A new business model or company
that can shake up the market will do it. If Ups success indicates anything it's
that if the banking industry can be distrupted, any industry can be.
...
[^3]:
    ...
    talks about a similar trend of millenials preferring to work at a
